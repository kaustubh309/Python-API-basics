...
def display_top_cryptos():
    ...
    print(f"{'=' * 55}")
def compare_cryptos():
    coins = input("Enter crypto names (comma separated): ").lower().split(",")
    ...
    for coin in coins:
        ...
        usd = data["quotes"]["USD"]
        print(f"  {data['name']:<15}${usd['price']:>12,.2f}  {usd['percent_change_24h']:+.2f}%")

    print(f"{'=' * 55}")

def create_post():
    """Create a post using POST request and save response to JSON file."""
    print("\n=== Create a New Post ===\n")
    ...
    try:
        ...
        print("Response saved to results.json")
        print(data)
