...
url = "https://jsonplaceholder.typicode.com/users/5"   # CHANGED URL
response = requests.get(url)

if response.status_code == 200:
    data = response.json()
    print("Phone Number:", data['phone'])   # ACCESS PHONE
else:
    print("User not found!")
#
# Exercise 3: Count how many comments are on post ID 1
#             URL: https://jsonplaceholder.typicode.com/posts/1/comments
url = "https://jsonplaceholder.typicode.com/posts/1/comments"   # CHANGED URL
response = requests.get(url)

if response.status_code == 200:
    data = response.json()
    print("Number of comments:", len(data))   # ACCESS LENGTH
else:
